...
//		if(_gmenu.getCheckedRows(0)!=''){
		...
		for(var i=0, len=_gmenu.getRowsNum(); i<len; i++){
			menu = new Object();
...
//			}
		...
		for(var i=0, len=_gpage.getRowsNum(); i<len; i++){
			page = new Object();
...
//		if(_gbutton.getCheckedRows(0)!=''){
		...
		for(var i=0, len=_gbutton.getRowsNum(); i<len; i++){
			button = new Object();
...
//            	console.log(JSON.stringify(role));
            	if(role.data!=null){
            		for(var i=0, len=role.data.length; i<len; i++){
            			_grole.addRow(i, [role.data[i].id,role.data[i].description], i);
	...
	try{
		$.ajax({
            ...
            success: function(objMenu) {
            	if(objMenu!=null){
            		for(var i=0, len=objMenu.data.length; i<len; i++){
            			_gmenu.addRow(i, [objMenu.data[i].status,objMenu.data[i].menuname,objMenu.data[i].idmenu], i);
	...
	try{
		$.ajax({
            ...
            success: function(objPage) {
            	if(objPage!=null){
            		for(var i=0, len=objPage.data.length; i<len; i++){
            			_gpage.addRow(i, [objPage.data[i].status,objPage.data[i].description,objPage.data[i].id], i);
	...
	try{
		$.ajax({
            ...
            success: function(objButton) {
            	if(objButton!=null){
            		for(var i=0, len=objButton.data.length; i<len; i++){
            			_gbutton.addRow(i, [objButton.data[i].status,objButton.data[i].description,objButton.data[i].id], i);
